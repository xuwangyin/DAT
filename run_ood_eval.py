...
def load_ood_yaml_config(config_file: str) -> dict:
    ...
    missing_fields = [
        field for field in ood_required_fields if field not in config
    ]
    if missing_fields:
        raise ValueError(
            f"Missing OOD-specific required fields in config: {missing_fields}"
        )
    ...
    if use_slurm:
        ...
        try:
            ...
            if process.returncode == 0:
                print(
                    f"Successfully completed OOD detection evaluation: {ckpt_path}"
                )
                return True
            else:
                print(
                    f"OOD detection evaluation failed with return code {process.returncode}: {ckpt_path}"
                )
                return False
...
def run_ood_evaluation_from_config(
    ...
    if skip_completed and check_ood_job_completed(log_file):
        if verbose:
            ...
            print(
                "Reason: Job already completed successfully (found 'OOD detection evaluation completed' in log file)"
            )
    ...
    if verbose:
        print(
            f"Submitting OOD detection job for checkpoint: {ckpt_path}, OOD dataset: {ood_dataset}"
        )
