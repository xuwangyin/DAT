...
sys.path.insert(0, "openimages/src")


def create_natural_subset_for_ood(
    download_dir: str = "./data/openimages",
    ...
    _ = downloader.analyze_label_distribution("train")
...
class OpenImagesSubsetDownloader:
    ...
    def analyze_label_distribution(self, split: str = "train") -> pd.DataFrame:
        ...
        annotations_file = (
            self.download_dir / f"{split}-annotations-human-imagelabels.csv"
        )
        ...
        class_df = pd.read_csv(
            class_desc_file, header=None, names=["LabelName", "DisplayName"]
        )
        ...
        print(
            f"Found {len(analysis)} unique labels across {len(df)} annotations"
        )
    ...
    def select_diverse_subset(
        ...
        print(
            f"Performing stratified sampling from {df['LabelName'].nunique()} label classes..."
        )
        ...
        with warnings.catch_warnings():
            ...
            sampled_df = (
                df.groupby("LabelName", group_keys=False)
                .apply(sample_group)
                .reset_index(drop=True)
            )

        print(
            f"Initial stratified sample: {len(sampled_df)} annotations from {sampled_df['LabelName'].nunique()} classes"
        )
        ...
        print(
            f"After removing duplicate images: {len(unique_images_df)} unique images"
        )

        # If we have more images than target, do final random sampling
        if len(unique_images_df) > target_images:
            final_sample = unique_images_df.sample(
                n=target_images, random_state=random_state
            )
        ...
        image_to_labels = (
            final_annotations.groupby("ImageID")["LabelName"]
            .apply(list)
            .to_dict()
        )
        ...
        print(
            f"Downloading {len(final_image_ids)} unique images with label suffixes..."
        )

        # Download each image once with concatenated labels
        downloaded_count = self.download_images_by_id_with_labels(
            savedir, image_to_labels, class_map
        )
        print(
            f"Download complete! {downloaded_count}/{len(final_image_ids)} images downloaded to: {savedir}"
        )
    ...
    def _print_sampling_stats_unique(
        self, final_sample: pd.DataFrame, original_sample: pd.DataFrame
    ):
        ...
        print(
            f"Deduplication ratio: {len(final_sample) / len(original_sample) * 100:.1f}%"
        )

        # Get label distribution for final unique images
        # Join back with original annotations to see label distribution
        annotations_file = (
            self.download_dir / "train-annotations-human-imagelabels.csv"
        )
        df = pd.read_csv(annotations_file)
        ...
        print(
            f"Annotations per class - Mean: {class_counts.mean():.1f}, "
            f"Std: {class_counts.std():.1f}"
        )
        print(
            f"Annotations per class - Min: {class_counts.min()}, "
    ...
    def download_images_by_human_labels(
        self,
        dest_dir: str,
        class_labels: list,
        label_code: str,
        ...
        class_images = df[
            (df["LabelName"] == label_code) & (df["Confidence"] == 1)
        ]
        ...
        logger.info(
            f"Found {len(image_ids)} images with human labels for {class_labels[0]}"
        )
    ...
    def _download_images_by_id_custom(
        self, image_ids: list, section: str, images_directory: str
    ):
        """
        ...
        logger.info(
            f"Downloading {len(image_ids)} images to {images_directory}"
        )

        with ThreadPoolExecutor(max_workers=4) as executor:
            # Submit all download tasks
            future_to_id = {
                executor.submit(download_single_image, img_id): img_id
    ...
    def download_images_by_label_direct(
        self, dest_dir: str, display_name: str, label_code: str, image_ids: list
    ):
        """
    ...
    def load_imagenet_classes(
        self, imagenet_json_path: str = "./imagenet_class_index.json"
    ) -> set:
        ...
        try:
            ...
            print(
                f"Loaded {len(imagenet_classes)} ImageNet class terms for filtering"
            )
    ...
    def download_images_by_id_with_labels(
        self, dest_dir: str, image_to_labels: dict, class_map: dict
    ):
        """
        ...
        def create_label_suffix(label_codes):
            ...
            for code in label_codes:
                ...
                if (
                    ...
                    and display_name_lower in self._imagenet_classes
                ):
                    return False  # Skip this image due to ImageNet overlap
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            future_to_id = {
                executor.submit(
                    download_single_image_with_suffix, img_id, labels
                ): img_id
            ...
            for future in as_completed(future_to_id):
                ...
                if processed % 10 == 0 or processed == total_images:
                    print(
                        f"Progress: {processed}/{total_images} processed, {successful} downloaded, {imagenet_filtered} filtered (ImageNet), {failed} failed"
                    )

        print(
            f"Final: {successful} downloaded, {imagenet_filtered} filtered out (ImageNet overlap), {failed} failed"
        )
        return successful
    ...
    def download_images_with_tool(
        self, image_ids: Set[str], split: str = "train"
    ):
        """
        ...
        try:
            ...
            print(
                "OpenImages downloader not found. Install with: pip install openimages"
            )
...
if __name__ == "__main__":
    ...
    if len(selected_images) < 50000:
        ...
        print(
            f"Your training set has 566K annotations → {len(selected_images)} unique images"
        )
        print(
            f"Deduplication ratio: {len(selected_images) / 566520 * 100:.1f}%"
        )

        print("\nTo get more images:")
        print(
            "1. Download the FULL OpenImages training set (9+ million annotations)"
        )
        print(
            "   This would provide 500K-1M+ unique images instead of your current subset"
        )
        print(
            "2. Your current data appears to be a subset/sample of the full OpenImages dataset"
        )
    ...
    print(
        f"- Result: {len(selected_images)} unique images from 566K annotations"
    )
    ...
    if len(selected_images) < 10000:
        ...
        print(
            "Consider downloading the full OpenImages training set for better results."
        )
