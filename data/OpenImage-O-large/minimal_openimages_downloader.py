...
def download_single_image(
    s3_client, image_id: str, section: str, output_dir: str
):
...
def download_images(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(
                download_single_image, s3_client, image_id, section, output_dir
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Download OpenImages dataset images by ID"
    )
    parser.add_argument(
        "ids_file", help="Text file containing image IDs (one per line)"
    )
    parser.add_argument(
        "output_dir", help="Directory to save downloaded images"
    )
    parser.add_argument(
        "--section",
        choices=["train", "validation", "test"],
        default="train",
        help="Dataset section (default: train)",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    download_images(
        args.ids_file, args.output_dir, args.section, args.max_workers
    )
