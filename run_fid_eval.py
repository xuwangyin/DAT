...
def load_fid_yaml_config(config_file: str) -> dict:
    ...
    missing_fields = [
        field for field in fid_required_fields if field not in config
    ]
    if missing_fields:
        raise ValueError(
            f"Missing FID-specific required fields in config: {missing_fields}"
        )
    ...
    if use_slurm:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                print(
                    f"FID evaluation failed with return code {process.returncode}: {ckpt_path}"
                )
                return False
...
def run_fid_evaluation_from_config(
    ...
    if verbose:
        print(
            f"Submitting job for checkpoint: {ckpt_path} with {num_steps} steps"
        )
