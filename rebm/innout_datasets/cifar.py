...
from .paths import get_base_data_dir, get_CIFAR10_path, get_CIFAR100_path
...
def GrayCIFAR10(
    train=False, batch_size=None, augm_flag=False, shuffle=None, resolution=28
):
    if batch_size == None:
        if train:
    ...
    transform_base = [
        transforms.Compose(
            [transforms.Resize(resolution), transforms.ToTensor(), Gray()]
        )
    ]
    transform_train = transforms.Compose(
        [
            ...
            transforms.RandomCrop(
                resolution, padding=4, padding_mode="reflect"
            ),
        ]
        + transform_base
    )
...
class CIFAR10_1Dataset(VisionDataset):
    def __init__(self, transform=None, target_transform=None, version="v6"):
...
class CIFAR100MinusCIFAR10(Dataset):
    def __init__(
        self, path, train=True, samples_per_class=None, transform=None
    ):
        super().__init__()
        ...
        print(
            f"Cifar100 Minus Cifar10 - Remaining classes: {len(self.classes)}"
        )
        ...
        for cls_idx, cls in enumerate(self.classes):
            ...
            class_idcs = torch.nonzero(
                cifar100_targets == cifar100_class_idx, as_tuple=False
            ).squeeze()
            cifar100_idcs.append(class_idcs)
            targets.append(
                cls_idx * torch.ones(len(class_idcs), dtype=torch.long)
            )

        self.cifar100_idcs = torch.cat(cifar100_idcs)

        if samples_per_class is None:
            self.subset_idcs = torch.arange(
                len(self.cifar100_idcs), dtype=torch.long
            )
            print(f"Using all {len(self.subset_idcs)} samples")
        else:
            split = "train" if train else "test"
            idcs_filename = (
                f"cifar100_minus_cifar10_{split}_{samples_per_class}.pt"
            )
            if os.path.exists(idcs_filename):
                self.subset_idcs = torch.load(idcs_filename)
                print(
                    f"Loading an existing subset of {len(self.subset_idcs)} samples"
                )
            else:
                ...
                for cls_idx, cls in enumerate(self.classes):
                    ...
                    selected_cls_idcs = torch.randperm(num_cls_samples)[
                        :samples_per_class
                    ]
                    self.subset_idcs[
                        cls_idx * samples_per_class : (cls_idx + 1)
                        * samples_per_class
                    ] = offset + selected_cls_idcs
...
class CIFAR10MinusCIFAR100(Dataset):
    def __init__(self, path, train=True, transform=None):
        ...
        print(
            f"Cifar10 Minus Cifar100 - Remaining classes: {len(self.classes)}"
        )
        ...
        for cls_idx, cls in enumerate(self.classes):
            ...
            class_idcs = torch.nonzero(
                cifar10_targets == cifar10_class_idx, as_tuple=False
            ).squeeze()
