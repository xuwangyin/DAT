...
def get_model(
    ...
    if "resnet" in model_type.lower() and model_type.lower().endswith("imagenet"):
        ...
        model = model_class(
            ...
            normalization_type=indist_dataset.lower(),
    ...
    # CIFAR ResNet/WideResNet models
    elif (
        model_type.lower().startswith(("resnet", "preactresnet", "wideresnet"))
        and not model_type.lower().endswith("imagenet")
    ):
        # Handle different types of ResNet models
        if model_type.lower().startswith("preact"):
            # For PreActResNet models, use the preactresnet module
            model_class = getattr(rebm.models.preactresnet, model_type)
            ...
            model_class = getattr(rebm.models.robustness_resnet_cifar10, model_type)
